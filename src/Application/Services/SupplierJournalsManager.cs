...
namespace Application.Services
{
	public class SupplierJournalsManager : ISupplierJournalsManager
	{
		...
		public SupplierJournalsManager(IApplicationDbContext db, IJournalManager journalManager,
		...
		public async Task<string> PurchaseJournalAsync(PurchaseContainer vm, Contacts Contact, IFormFile Invoice)
		{
		...
		public async Task<string> ExpenseJounralAsync(ExpenseVM vm, string ExpenseAccNum, Contacts Contact, Currency Currency)
		{
		...
		public async Task<string> SupplierPaymentJournalAsync(SupplierPaymentContainer vm, Contacts Contact, decimal LocalAmount)
		{
			var journal = new JournalVM();
			journal.TransDate = vm.PaymentDetails.PaymentDate;
			journal.TransDesc = vm.PaymentDetails.Description;
		...
		public async Task<string> PurchaseReturnJournalAsync(PurchaseReturnBackContainer vm, Contacts Contact, Currency Currency, bool IsVAT, decimal TotalInLocal, decimal TotalAmountWithVat)
		{
			var journal = new JournalVM();
