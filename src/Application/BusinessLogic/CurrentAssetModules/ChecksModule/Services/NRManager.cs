...
namespace Application.BusinessLogic.CurrentAssetModules.ChecksModule.Services
{
	public class NRManager
	{
		...
		public void MoveToBank(CheckInSafeContainer vm)
		{
			using (var transaction = _db.Database.BeginTransaction())
			{
				try
				{
					...
					// Journal Transaction
					var TransactionId = MoveToBankJournal(vm, CheckTotalAmount);
					// Update Check Table
		...
		private string MoveToBankJournal(CheckInSafeContainer vm, decimal CheckTotalAmount)
		{
			...
			var TransactionId = _journal.SaveJournal(journal);
		...
		public void CollectCheck(CheckInBankContainer vm)
		{
			using (var transaction = _db.Database.BeginTransaction())
			{
				try
				{
					...
					var CheckTotalAmount = SelectedChecks.Sum(x => x.CheckAmount);
					// Journal Transaction
					var TransactionId = CollectionJournal(SelectedChecks);
					// Update Check Table
